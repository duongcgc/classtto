<?php
/**
 * The Classtto HTML component contains a powerful set of functions to create flexible and easy overwritable HTML markup,
...
class Html {
	...
	public function __construct() {
		...
		/**
		 * Fires before the html is loaded.
		 *
		 * @since 1.0.0
		 */
		do_action( 'ctto_before_load_html' );

		/**
		 * Fires when the html loads.
		 *
		 * This hook is the root of Classtto's framework hierarchy. It all starts here!
		 *
		 * @since 1.0.0
		 */
		do_action( 'ctto_load_html' );

		/**
		 * Fires after the html is loaded.
		 *
		 * @since 1.0.0
		 */
		do_action( 'ctto_after_load_html' );
	}
	 ...
	 *      2. removed by using {@see ctto_remove_output()}.
	 *
	 ...
	 *
	 * Notes:
	 *      1. Since this function uses {@see apply_filters()}, the $id argument may contain sub-hook(s).
	 ...
	 * @return string|void
	 */
	public static function ctto_output( $id, $output ) {
		$args    = func_get_args();
		$args[0] = $id . '_output';

		$output = call_user_func_array( 'apply_filters', $args );
	...
	/**
	 * Register and then echo the output for the given ID.  This function is a wrapper for {@see ctto_output()}.  See
	 * {@see ctto_output()} for more details.
	 *
	 * @since 1.0.0
	 * @uses  ctto_output()  To register output by ID.
	 *
	 * @param string $id     A unique string used as a reference. The $id argument may contain sub-hook(s).
	 * @param string $output The given content to output.
	 */
	 ...
	 * @return string|void
	 */
	public static function ctto_open_markup( $id, $tag, $attributes = array() ) {
		$args            = func_get_args();
		...
		// Filter the tag.
		$tag = call_user_func_array( 'apply_filters', $args );
		...
		global $_ctto_is_selfclose_markup;
		...
		$output  = call_user_func_array( '_ctto_render_action', $args );
		...
		$args[0] = $id . ( $_ctto_is_selfclose_markup ? '_after_markup' : '_prepend_markup' );
		$output .= call_user_func_array( '_ctto_render_action', $args );

		// Reset the global variable to reduce memory usage.
		unset( $GLOBALS['_ctto_is_selfclose_markup'] );

		return $output;
	}

	/**
	 * Echo the opening HTML tag's markup.  This function is a wrapper for {@see ctto_open_markup()}.  See
	 * {@see ctto_open_markup()} for more details.
	 *
	 * @since 1.0.0
	 *
	 * @param string       $id         A unique string used as a reference. The $id argument may contain sub-hooks(s).
	 ...
	 * @return void
	 */
	public static function ctto_open_markup_e( $id, $tag, $attributes = array() ) {
		$args = func_get_args();
		echo call_user_func_array( 'ctto_open_markup', $args ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped -- Escaped in ctto_open_markup().
	}

	/**
	 * Build the self-closing HTML element's markup.
	 *
	 * This function is shortcut of {@see ctto_open_markup()}. It should be used for self-closing HTML markup such as
	 * images or inputs.
	 *
	 ...
	 * @since 1.0.0
	 *
	 * @global bool        $_ctto_is_selfclose_markup When true, indicates a self-closing element should be built.
	 *
	 * @param string       $id         A unique string used as a reference. The $id argument may contain sub-hooks(s).
	 ...
	 * @return string|void
	 */
	public static function ctto_selfclose_markup( $id, $tag, $attributes = array() ) {
		global $_ctto_is_selfclose_markup;

		$_ctto_is_selfclose_markup = true;

		$html = call_user_func_array( 'ctto_open_markup', func_get_args() );
	...
	/**
	 * Echo the self-closing HTML element's markup. This function is a wrapper for {@see ctto_selfclose_markup()}.  See
	 * {@see ctto_selfclose_markup()} for more details.
	 *
	 * @since 1.0.0
	 *
	 * @param string       $id         A unique string used as a reference. The $id argument may contain sub-hooks(s).
	 ...
	 * @return void
	 */
	public static function ctto_selfclose_markup_e( $id, $tag, $attributes = array() ) {
		echo call_user_func_array( 'ctto_selfclose_markup', func_get_args() ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped -- Escaped in ctto_open_markup().
	}
	 ...
	 * @return string|void
	 */
	public static function ctto_close_markup( $id, $tag ) {
		// Filter the tag.
		$tag = apply_filters( $id . '_markup', $tag );
	...
	/**
	 * Echo the closing HTML tag's markup.  This function is a wrapper for {@see ctto_close_markup()}.  See
	 * {@see ctto_close_markup()} for more details.
	 *
	 * @since 1.0.0
	 *
	 * @param string $id  Identical to the opening markup ID.
	 ...
	 * @return void
	 */
	public static function ctto_close_markup_e( $id, $tag ) {
		$args = func_get_args();
		echo call_user_func_array( 'ctto_close_markup', $args ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped -- Escaped in ctto_close_markup().
	}
	 ...
	 */
	public static function ctto_modify_markup( $id, $markup, $priority = 10, $args = 1 ) {
		return add_filter( $id . '_markup', $markup, $priority, $args );
	}
	 ...
	 * @return bool|_CTTO_Anonymous_Filters
	 */
	public static function ctto_remove_markup( $id, $remove_actions = false ) {
		return add_filter( $id . '_markup', $remove_actions ? null : false );
	}
	 ...
	 * @return void
	 */
	public static function ctto_reset_markup( $id ) {
	 ...
	 *
	 *    1. `$id_before_markup`:  When this hook fires, {@see ctto_open_markup()} is called to build the wrap's
	 *        opening HTML markup.
	 *    2. `$id_after_markup`: When this hook fires, {@see ctto_close_markup()} is called to build the wrap's
	 *        closing HTML markup.
	 *
	 * Note: You can pass additional arguments to the functions that are hooked to <tt>$id</tt>.
	 *
	 ...
	 * @return bool
	 */
	public static function ctto_wrap_markup( $id, $new_id, $tag, $attributes = array() ) {
	 ...
	 * This function registers an anonymous callback to the following action hooks:
	 *
	 *    1. `$id_prepend_markup`:  When this hook fires, {@see ctto_open_markup()} is called to build the wrap inner
	 *        content's HTML markup just after the wrap's opening tag.
	 *    2. `$id_append_markup`: When this hook fires, {@see ctto_close_markup()} is called to build the wrap inner
	 ...
	 * @return bool
	 */
	public static function ctto_wrap_inner_markup( $id, $new_id, $tag, $attributes = array() ) {
	 ...
	 *
	 * Since this function uses {@see apply_filters()}, the $id argument may contain sub-hook(s).
	 *
	 * Note: You can pass additional arguments to the functions that are hooked to <tt>$id</tt>.
	 *
	 * @since 1.0.0
	 ...
	 */
	public static function ctto_add_attributes( $id, $attributes = array() ) {
		$args    = func_get_args();
		...
		$attributes = call_user_func_array( 'apply_filters', $args );

		return ctto_esc_attributes( $attributes );
	}
	 ...
	 * @return void
	 */
	public static function ctto_reset_attributes( $id ) {
	...
	public static function ctto_is_html_dev_mode() {
		...
		return (bool) ctto_get_option( 'dev_mode', false );
	}
