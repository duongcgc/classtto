 ...
 *
 * @package Classtto\Framework\API\Utilities
 *
 * @since   1.5.0
 */
 ...
 */
// require_once CTTO_INC_PATH . 'utilities/plugins.php';
 ...
 */
function ctto_render_function($callback) {
 ...
 * @return string The callback content.
 */
function ctto_render_function_array($callback, $params = array()) {
 ...
 */
function ctto_remove_dir($dir_path) {
...
function ctto_path_to_url($path, $force_rebuild = false) {
    ...
    if (!$root_path || true === $force_rebuild) {
        ...
        $maybe_tilde = ctto_get(0, explode('/', trailingslashit(ltrim($subfolder, '/'))));
    ...
    } elseif (false !== stripos($path, ctto_get('DOCUMENT_ROOT', $_SERVER))) {
        $path = str_replace(ctto_get('DOCUMENT_ROOT', $_SERVER), '', $path);
    }
 ...
 * @SuppressWarnings(PHPMD.NPathComplexity)
 */
function ctto_url_to_path($url, $force_rebuild = false) {
    ...
    $maybe_tilde  = ctto_get(0, explode('/', $trimmed_path));
    ...
    if (isset($set_root)) {
        $root_path = ctto_sanitize_path($root_path);
    }
    ...
    return ctto_sanitize_path($path);
}
 ...
 */
function ctto_sanitize_path($path) {
 ...
 */
function ctto_get($needle, $haystack = false, $default = null) {
 ...
 * @return string Returns the value if found; else $default is returned.
 */
function ctto_post($needle, $default = null) {
    return ctto_get($needle, $_POST, $default); // phpcs:ignore WordPress.CSRF.NonceVerification.NoNonceVerification -- The nonce verification check should be at the form processing level.
}
 ...
 * @return string Returns the value if found; else $default is returned.
 */
function ctto_get_or_post($needle, $default = null) {
    $get = ctto_get($needle);
 ...
 * @param bool   $strict   If the third parameter strict is set to true, the ctto_in_multi_array()
 *                         function will also check the types of the needle in the haystack.
 *
 * @return bool Returns true if needle is found in the array; else, false is returned.
 */
function ctto_in_multi_array($needle, $haystack, $strict = false) {
    ...
    foreach ((array) $haystack as $value) {
        ...
        if (is_array($value) && ctto_in_multi_array($needle, $value)) {
            return true;
        }
    }
 ...
 * @return bool Returns true if needle is found in the array; else, false is returned.
 */
function ctto_multi_array_key_exists($needle, array $haystack) {
    ...
    foreach ($haystack as $value) {
        ...
        if (is_array($value) && ctto_multi_array_key_exists($needle, $value)) {
            return true;
        }
    }
 ...
 */
function ctto_array_shortcodes($content, $haystack) {
 ...
 */
function ctto_admin_menu_position($position) {
    global $menu;
    ...
    if (array_key_exists($position, $menu)) {
        return ctto_admin_menu_position($position + 1);
    }
 ...
 * @SuppressWarnings(PHPMD.ElseExpression)
 */
function ctto_esc_attributes($attributes) {
     ...
     */
    $methods = apply_filters('ctto_escape_attributes_methods', array(
        'href'     => 'esc_url',
        'src'      => 'esc_url',
        'itemtype' => 'esc_url',
        'onclick'  => 'esc_js',
    ...
    foreach ((array) $attributes as $attribute => $value) {
        ...
        $method = ctto_get($attribute, $methods);
 ...
 * @return bool
 */
function _ctto_is_uri($maybe_uri) {
 ...
 * @return bool
 */
function ctto_is_archive($post_type = 'post') {
 ...
 * @return bool
 */
function ctto_get_custom_template($id) {
 ...
 */
function ctto_post_element_enabled($element) {

    $elements = [];
    $field    = ctto_get_field("ctto_post_{$element}", 'global');
    ...
    $elements = ctto_get_post_single_elements();
 ...
 */
function ctto_get_post_single_elements() {
    $elements = [];

    if (is_singular('post')) {
        $elements = get_theme_mod('ctto_post_single_elements', [
            'featured_image',
            'date',
            'author',
            'categories',
        ...
        $elements = get_theme_mod('ctto_portfolio_single_elements', [
            'featured_image',
            'categories',
            'social_share',
            'related_posts',
        ...
        $elements = get_theme_mod('ctto_page_single_elements', [
            'featured_image',
            'comments',
        ]);
    } elseif (is_singular() && !is_singular('page')) {
        $elements = get_theme_mod('ctto_' . get_post_type() . '_single_elements', [
            'featured_image',
        ]);
    }
 ...
 * @SuppressWarnings(PHPMD.NPathComplexity)
 */
function ctto_get_current_page_key() {
    $ctto_cpt_archives = ctto_get_post_types_archives();

    foreach ($ctto_cpt_archives as $ctto_cpt_archive => $ctto_cpt_archive_label) {
        if (is_post_type_archive(str_replace('archive__', '', $ctto_cpt_archive))) {
            return $ctto_cpt_archive;
        }
    }
    ...
    // Custom post type.
    if (in_array(get_post_type(), ctto_get_post_types(), true)) {
 ...
 * @return array
 */
function ctto_get_exception_mod($setting) {
    ...
    $page_key = ctto_get_current_page_key();
 ...
 */
function ctto_get_direction($direction) {
    if (!is_rtl()) {
        return $direction;
    }
 ...
 * @return void
 */
function ctto_the_help_link($href = '#', $title = '', $theme = 'light') {
    echo ctto_get_the_help_link($href, $title, $theme); //phpcs:ignore
}
 ...
 */
function ctto_get_the_help_link($href = '#', $title = '', $theme = 'light') {
    if (!ctto_is_help_links()) {
        return '';
    }

    $theme = 'dark' === $theme ? 'dark' : 'light';
    return '<a href="' . esc_url($href) . '" class="ctto-help-link ctto-help-link-' . esc_attr($theme) . ' ctto-icon-question" title="' . esc_attr($title) . '" target="_blank"></a>';
}
 ...
 */
function ctto_is_callable($callable) {
 ...
 */
function ctto_upgrade_link($utm = '') {
    $base_link = 'https://themeforest.net/item/jupiter-multipurpose-responsive-theme/5177775?ref=artbees';
    switch ($utm) {
        case 'comparison':
            return $base_link . '&utm_source=FreeVSProPageUpgradeCTA&utm_medium=AdminUpgradePopup&utm_campaign=FreeClassttoAdminUpgradeCampaign';
        case 'customizer':
            return $base_link . '&utm_source=CustomizerAreaUpgradeButton&utm_medium=AdminUpgradePopup&utm_campaign=FreeClassttoAdminUpgradeCampaign';
        case 'banner':
            return $base_link . '&utm_source=BannerAdUpgradeCTA&utm_medium=AdminUpgradePopup&utm_campaign=FreeClassttoAdminUpgradeCampaign';
        case 'templates':
            return $base_link . '&utm_source=TemplateSectionImportButton&utm_medium=AdminUpgradePopup&utm_campaign=FreeClassttoAdminUpgradeCampaign';

        default:
            return $base_link . '&utm_medium=AdminUpgradePopup&utm_campaign=FreeClassttoAdminUpgradeCampaign';
    }
}
 ...
 */
function ctto_dynamic_remove_action($tag, $function_name, $priority = null) {
    if (!is_null($priority)) {
 ...
 */
function ctto_get_custom_post_types($output = 'names') {
    $args = [
        'public'             => true,
 ...
 * @SuppressWarnings(PHPMD.ElseExpression)
 */
function ctto_get_post_types($output = 'names', $args = []) {
    $post_types = ctto_get_option('post_types');
 ...
 * @SuppressWarnings(PHPMD.ElseExpression)
 */
function ctto_get_post_types_archives() {
    $post_types = ctto_get_post_types('labels');
    $archives   = [];
    ...
    foreach ($post_types as $type => $label) {
        ...
        $archives['archive__' . $type] = $label . esc_html__(' Archive', 'ctto');
    }
 ...
 * @SuppressWarnings(PHPMD.NPathComplexity)
 */
if (!function_exists('ctto_get_archive_post_type')) {
    /**
     ...
     * @return string
     */
    function ctto_get_archive_post_type() {
        $post_type = '';
 ...
 */
function ctto_get_option($option, $default = false) {
    $options = get_option('ctto', []);
 ...
 */
function ctto_update_option($option, $value) {
    $options = get_option('ctto', []);
 ...
 */
function ctto_delete_option($option) {
    $options = get_option('ctto', []);
 ...
 */
function ctto_is_white_label() {
    if (!ctto_is_callable('ctto_core')) {
        return false;
    }

    if (!ctto_get_option('white_label')) {
        return false;
    }
 ...
 */
function ctto_is_help_links() {
    if (!ctto_is_white_label() || (ctto_is_white_label() && ctto_get_option('white_label_help_links', true))) {
        return true;
    }
 ...
 * @since 1.21.0
 */
if (!function_exists('ctto_wpcolorpickeralpha_localize')) {
    /**
     * Get archive post type. If there are multiple post types on the archive template then return empty.
     *
     * @since 1.21.0
     ...
     * @return void
     */
    function ctto_wpcolorpickeralpha_localize() {
        $color_picker_strings = array(
            'clear'            => __('Clear', 'ctto-core'),
            'clearAriaLabel'   => __('Clear color', 'ctto-core'),
            'defaultString'    => __('Default', 'ctto-core'),
            'defaultAriaLabel' => __('Select default color', 'ctto-core'),
            'pick'             => __('Select Color', 'ctto-core'),
            'defaultLabel'     => __('Color value', 'ctto-core'),
        );
